# Converted from 03_rendering_comparison_schemeA.ipynb
...
#
# The backtest in this notebook now follows **Scheme A**:
#
# - the model generates a raw signal on date `t`,
# - if the strategy enters, it holds the CSI300 for the next **5 trading days**,
# - all signals that arrive during those 5 holding days are ignored.
# ==============================================================================
...
def render_ohlc_image(window, height=64, include_ma=False, include_volume=False,
                      volume_ratio=0.2):
    '''
    Render OHLC image using the EXACT same drawing logic as the stronger 2D CNN notebook
    whenever MA / volume are included:
    - OHLC wick/ticks in white (255)
    - MA drawn as a single point at x0+1 with grayscale 180 (NOT a connected line)
    - volume bars drawn with grayscale 120
    '''
    ...
    for i in range(n_days):
        ...
        # Stronger notebook OHLC rendering
        ...
        if include_ma:
            ...
            # EXACT stronger notebook style: one MA point, not a connected line
            img[y_ma, x0+1] = 180

        if include_volume and vol_h > 0:
            # EXACT stronger notebook volume style
            bar_h = int(round(vol_[i] / max_vol * max(1, vol_h - 1)))
            if bar_h > 0:
                img[price_h + (vol_h - bar_h):height, x0:x0+3] = 120
...
def render_candlestick_image(window, height=64, include_ma=True, include_volume=True,
    ...
    For consistency with the stronger notebook alignment request:
    - MA is drawn as a single point (not a connected line)
    - volume bars use grayscale 120
    '''
    n_days = window.shape[0]
    width = n_days * 3
    img = np.zeros((height, width), dtype=np.uint8)
...
# ==============================================================================
# ## Interpretation of this notebook
#
# The purpose of this notebook is to test whether prediction performance changes when the **image rendering rule** changes, while everything else stays fixed:
#
# - the same 20-day input window,
# - the same future 5-day classification target,
# - the same train / validation / test split,
# - the same 2D CNN architecture,
# - the same Scheme A backtest with a 5-day silent holding rule.
#
# Therefore, any performance differences here should mainly come from the **image representation itself**, not from a different model or a different label definition.
# ==============================================================================
